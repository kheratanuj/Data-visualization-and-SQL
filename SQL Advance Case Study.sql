...
select distinct State from FACT_TRANSACTIONS F inner join DIM_LOCATION L on F.IDLocation=L.IDLocation
where year(Date)>=2005

--2.	What state in the US is buying more 'Samsung' cell phones?
select TOP 1
State,count(Manufacturer_Name)as No_of_sales from 
FACT_TRANSACTIONS F inner join DIM_MODEL M2 on F.IDModel=M2.IDModel
inner join DIM_MANUFACTURER M1 on M2.IDManufacturer=M1.IDManufacturer
...
select State,ZipCode,Model_Name,count(Model_Name) as [Number of Transactions] 
from FACT_TRANSACTIONS F inner join DIM_MODEL M2 on F.IDModel=M2.IDModel
inner join DIM_LOCATION L on F.IDLocation=L.IDLocation
group by State,ZipCode,Model_Name

--4.	Show the cheapest cellphone
select Manufacturer_Name,Model_Name,Unit_price from DIM_MODEL M2 inner join DIM_MANUFACTURER M1 on M2.IDManufacturer=M1.IDManufacturer
...
select Manufacturer_Name,Model_Name,AVG(TotalPrice) as Average_Price from FACT_TRANSACTIONS F inner join 
DIM_MODEL M2 on F.IDModel=M2.IDModel
inner join DIM_MANUFACTURER M1 on M2.IDManufacturer=M1.IDManufacturer
where Manufacturer_Name in (
select Manufacturer_Name from (
select TOP 5
M1.IDManufacturer,M1.Manufacturer_Name,sum(Quantity) as [Total quantity] from 
FACT_TRANSACTIONS F inner join DIM_MODEL M2 on F.IDModel=M2.IDModel
inner join DIM_MANUFACTURER M1 on M2.IDManufacturer=M1.IDManufacturer
group by M1.IDManufacturer,M1.Manufacturer_Name
) as T1
...
select Customer_Name,avg(TotalPrice) as Average_Spend from FACT_TRANSACTIONS F inner join DIM_CUSTOMER C
on F.IDCustomer=C.IDCustomer where year(Date)=2009
...
select Model_Name from 
		(select top 5
		Model_Name,sum(Quantity) As Quantity from FACT_TRANSACTIONS F inner join DIM_MODEL M2 on F.IDModel=M2.IDModel
		where year(Date) in ('2008')
		group by Model_Name  
		order by Quantity Desc) as Y1
INTERSECT
select Model_Name from 
		(select top 5
		Model_Name,sum(Quantity) As Quantity from FACT_TRANSACTIONS F inner join DIM_MODEL M2 on F.IDModel=M2.IDModel
...
select * from
(select top 1 Manufacturer_Name,YEAR,Total_amount from
(select top 2 Manufacturer_Name,year(Date) as YEAR,sum(TotalPrice) as Total_amount from FACT_TRANSACTIONS F inner join DIM_MODEL M2 on F.IDModel=M2.IDModel
inner join DIM_MANUFACTURER M1 on M2.IDManufacturer=M1.IDManufacturer
where year(date)='2009'
group by Manufacturer_Name,year(Date)
order by Total_amount desc) as Y1
order by Total_amount) as Y3
...
group by Manufacturer_Name,year(Date)
order by Total_amount desc) as Y2
order by Total_amount) as Y4

--9.	Show the manufacturers that sold cellphone in 2010 but didn’t in 2009.
select  distinct Manufacturer_Name from FACT_TRANSACTIONS F inner join DIM_MODEL M2 on F.IDModel=M2.IDModel
inner join DIM_MANUFACTURER M1 on M2.IDManufacturer=M1.IDManufacturer
where year(date)='2010'
...
select TOP 100
Customer_Name,year(Date) as YEAR, sum(TotalPrice) as Total_Spend, avg(TotalPrice) as Average_Spend,avg(Quantity) as Average_Quantity from FACT_TRANSACTIONS F inner join DIM_CUSTOMER C
on F.IDCustomer=C.IDCustomer
group by C.Customer_Name,year(Date)
